...
$plan = [ordered]@{
    ...
    fixture = $fixture; tuples = $tuples; run_order = $order; active_pause_run = 7
    ...
    maximum_generated_source_bytes = 7 * $fixture.bytes
    thresholds = Get-RecoveryThresholds
    active_pause_thresholds = @{ response_ms = 150; extraction_drain_seconds = 5; hold_seconds_minimum = 3 }
}
...
try {
    for ($run = 0; $run -lt $order.Count; $run++) {
        ...
        $checkPause = $run -eq 6
        Write-Host "Workload run $($run + 1)/7: $($tuple.workers)/$($tuple.scan_threads)/$($tuple.concurrent_scans)/$($tuple.device_readers); active pause=$checkPause"
        ...
        Write-Host "Run $($run + 1): pass=$passed; failed=$($evaluation.failed_checks -join ', '); active-pause=$($pauseEvaluation.failed_checks -join ', '); rejected=$($evaluation.rejected_because)"
    }
